...
function fetchUrl(url) {
  return new Promise((resolve, reject) => {
    ...
    const req = mod.get(url, { headers: { 'User-Agent': 'AnayaPropertyLookup/1.0' } }, (res) => {
      let data = '';
...
app.get('/property', async (req, res) => {
  ...
  try {
    ...
    if (county === 'la') {
      ...
      const ARCGIS = {
        riverside:     { base: 'https://gis.rctlma.org/arcgis/rest/services/Assessor/Parcels/MapServer/0', addrField: 'SITUS_ADDR' },
        orange:        { base: 'https://services1.arcgis.com/P5Mv5GY5S66M8Z1Q/arcgis/rest/services/Parcels_Public/FeatureServer/0', addrField: 'SitusAddress' },
        sanbernardino: { base: 'https://gis.sbcounty.gov/sbcgis/rest/services/Assessor/Parcels/MapServer/0', addrField: 'SITE_ADDRESS' },
      };
      const cfg = ARCGIS[county];
      if (cfg) {
        ...
        const url = cfg.base + '/query?' + new URLSearchParams({ where, outFields: '*', f: 'json', resultRecordCount: 1 }).toString();
        const data = await fetchUrl(url);
