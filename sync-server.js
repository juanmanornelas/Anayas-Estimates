...
function fetchUrl(url) {
  return new Promise((resolve, reject) => {
    ...
    const req = mod.get(url, { headers: { 'User-Agent': 'AnayaPropertyLookup/1.0', 'Accept': 'application/json' } }, (res) => {
      let data = '';
      ...
      res.on('end', () => {
        ...
        catch(e) { reject(new Error('Non-JSON response: ' + data.slice(0, 60))); }
      });
    });
    req.on('error', reject);
...
function detectCounty(zip, addr) {
  ...
  return 'riverside'; // default for this company's area
}
...
app.get('/property', async (req, res) => {
  ...
  try {
    ...
    if (county === 'la') {
      const houseNum = address.replace(/[^0-9]/g, '').slice(0, 6);
      const streetName = address.replace(/^\d+\s*/, '').split(' ')[0];
      const url = 'https://assessor.lacounty.gov/commondata/OutputData/SearchExistingAccounts?' +
        new URLSearchParams({ houseNum, streetName, unitNum: '', searchType: 'address' }).toString();
      ...
      if (rec) {
        ...
        if (rec.Pool) det.pool = rec.Pool === 'Y' || rec.Pool === 'Yes' || rec.Pool === 1;
        if (rec.UseType || rec.UseCode || rec.LandUse) det.useType = rec.UseType || rec.UseCode || rec.LandUse;
      ...
      const ARCGIS_CANDIDATES = {
        riverside: [
          { base: 'https://gis.rctlma.org/arcgis/rest/services/Assessor/Parcels/MapServer/0', addrField: 'SITUS_ADDR' },
          { base: 'https://mapping.rctlma.org/arcgis/rest/services/Assessor/Parcels/MapServer/0', addrField: 'SITUS_ADDR' },
        ],
        orange: [
          { base: 'https://services1.arcgis.com/P5Mv5GY5S66M8Z1Q/arcgis/rest/services/Parcels_Public/FeatureServer/0', addrField: 'SitusAddress' },
        ],
        sanbernardino: [
          { base: 'https://gis.sbcounty.gov/sbcgis/rest/services/Assessor/Parcels/MapServer/0', addrField: 'SITE_ADDRESS' },
        ],
      };
      const candidates = ARCGIS_CANDIDATES[county] || [];
      for (const cfg of candidates) {
        try {
          const houseNum  = address.replace(/\D/g, '').slice(0, 6);
          const streetName = address.replace(/^\d+\s*/, '').trim().toUpperCase();
          const where = `${cfg.addrField} LIKE '${houseNum} ${streetName.split(' ')[0]}%'`;
          // Build URL manually to avoid double-encoding of single quotes in WHERE clause
          const url = cfg.base + '/query?where=' + encodeURIComponent(where) + '&outFields=*&f=json&resultRecordCount=1';
          const data = await fetchUrl(url);
          const feat = (data.features || [])[0];
          if (feat && feat.attributes) {
            const a = feat.attributes;
            // Normalize common field names across counties
            det.sqft      = parseInt(a.SQ_FOOTAGE || a.BldgSqFt || a.BLDG_SQFT || a.SQFT || 0) || undefined;
            det.lotSqft   = parseInt(a.LOT_SQFT   || a.LotSqFt  || a.LOTSQFT   || 0)   || undefined;
            det.yearBuilt = parseInt(a.YR_BLT     || a.YearBuilt|| a.YEAR_BUILT || 0)   || undefined;
            det.bedrooms  = parseInt(a.BEDROOMS   || a.Bedrooms || a.BED_ROOMS  || 0)   || undefined;
            det.bathrooms = parseFloat(a.BATHROOMS || a.Bathrooms || a.BATH     || 0)   || undefined;
            det.stories   = parseInt(a.STORIES    || a.Stories  || a.NO_STORIES || 0)   || undefined;
            det.apn       = String(a.APN || a.PARCEL_NO || a.PARCELNO || a.PARCEL_ID || '').trim() || undefined;
            det.useType   = a.USECODE || a.UseCode || a.USE_CODE || a.LAND_USE || undefined;
          ...
          console.log('[property] candidate failed:', cfg.base, loopErr.message);
          // try next candidate
        }
      }
    }
