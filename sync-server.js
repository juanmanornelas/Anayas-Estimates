...
// ── Property Card Cache (shared across all admins) ───────────────────────────
const PROP_CACHE_FILE = path.join(DATA_DIR, 'anaya-property-cache.json');
function readPropCache() {
  if (!fs.existsSync(PROP_CACHE_FILE)) return {};
  try { return JSON.parse(fs.readFileSync(PROP_CACHE_FILE, 'utf8')); }
  catch(e) { return {}; }
}
function writePropCache(store) {
  fs.writeFileSync(PROP_CACHE_FILE, JSON.stringify(store), 'utf8');
}

// GET /property-cache  — returns all cached properties
app.get('/property-cache', (req, res) => {
  try { res.json({ ok: true, cache: readPropCache() }); }
  catch(e) { res.status(500).json({ error: e.message }); }
});

// POST /property-cache  { address, result, fullAddress }
app.post('/property-cache', (req, res) => {
  try {
    const { address, result, fullAddress } = req.body;
    if (!address || !result) return res.status(400).json({ error: 'Missing address or result' });
    const cache = readPropCache();
    const key = address.toLowerCase().trim();
    cache[key] = { address, fullAddress: fullAddress||address, result, cachedAt: new Date().toISOString(), cachedBy: req.body.cachedBy||'Admin' };
    // Keep max 100 entries — drop oldest
    const entries = Object.entries(cache).sort((a,b) => new Date(b[1].cachedAt) - new Date(a[1].cachedAt));
    const trimmed = Object.fromEntries(entries.slice(0, 200));
    writePropCache(trimmed);
    console.log('[prop-cache] saved:', fullAddress||address);
    res.json({ ok: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// DELETE /property-cache/:key
app.delete('/property-cache/:key', (req, res) => {
  try {
    const cache = readPropCache();
    delete cache[decodeURIComponent(req.params.key).toLowerCase()];
    writePropCache(cache);
    res.json({ ok: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
